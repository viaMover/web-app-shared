...
const CurrencyShortFormException: Record<string, string> = {
  BRL: 'R$',
  HKD: 'HK$'
};

export class MoneyFormatter {
  constructor(private readonly locale: string, private readonly defaultCurrency: string) {}

  formatCurrency(
    amount: number,
    form: 'explicit' | 'short' | 'short-explicit',
    ...
    switch (form) {
      ...
      case 'short-explicit':
        return this.formatCurrencyExplicit(amount, options).replace(
          this.getCurrencySymbol(this.locale, { ...options, currency: this.defaultCurrency })
            .symbol,
          ''
        );
    }
  ...
  private formatCurrencyExplicit(amount: number, options: NumberFormatOptions = {}): string {
  ...
  private getShortCurrencySymbol(
    ...
    try {
      shortSymbolResult = this.getCurrencySymbol(locale, {
        currency,
        currencyDisplay: 'narrowSymbol'
      });
    } catch {
      shortSymbolResult = this.getCurrencySymbol(locale, { currency });
    }
  ...
  getCurrencySymbol(locale: string, options: Intl.NumberFormatOptions) {
    const delimiters = ',.';
    const directionControlCharacters = /[\u200E\u200F]/;
    const numReg = new RegExp(`0[${delimiters}]*0*`);

    const currencyStringRaw = this.internalFormatCurrency(0, locale, options);
  ...
  private internalFormatCurrency(
    amount: number,
    locale: string,
